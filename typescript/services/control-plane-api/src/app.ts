...
const findDeploymentForTenant = async (
  repositories: CipRepositories,
  deploymentId: string,
) => {
...
const findApprovalRequestForTenant = async (
  repositories: CipRepositories,
...
export const createControlPlaneApiApp = (
  ...
  app.post("/v1/sessions/:sessionId/approval-requests", async (request, reply) => {
    try {
      ...
      const normalizedBody: RequestHumanApprovalInput = {
        ...body,
        actor: sdkActorFor(apiKey),
      };
      await reply.code(200).send(
        await options.controlPlane.requestHumanApproval(normalizedBody),
      );
    } catch (error) {
      await sendError(error, reply);
    }
  ...
  app.post(
    "/v1/approval-requests/:approvalRequestId(^[^:]+)::resolve",
    async (request, reply) => {
    try {
      ...
      const approvalRequest = await findApprovalRequestForTenant(
      ...
      const normalizedBody: ResolveApprovalRequestInput = {
        ...body,
        actor: {
          type: "human",
          id: claims.sub,
      ...
      await reply.code(200).send(
        await options.controlPlane.resolveApprovalRequest(normalizedBody),
      );
    } catch (error) {
      await sendError(error, reply);
    }
  ...
  app.post(
    "/v1/deployments/:deploymentId(^[^:]+)::transition",
    async (request, reply) => {
    try {
      const claims = await authenticateOperator(
        headerValue(request.headers.authorization),
      ...
      const deployment = await findDeploymentForTenant(
        options.repositories,
        params.deploymentId,
      );
      ...
      const normalizedBody: TransitionDeploymentInput = {
        ...body,
        actor: {
          type: "human",
          id: claims.sub,
      ...
      await reply.code(200).send(
        await options.controlPlane.transitionDeployment(normalizedBody),
      );
    } catch (error) {
      await sendError(error, reply);
    }
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        ...
        const normalizedBody: RollbackDeploymentInput = {
          ...body,
          actor: {
            type: "human",
            id: claims.sub,
        ...
        await reply.code(200).send(
          await options.controlPlane.rollbackDeploymentToBlueprint(normalizedBody),
        );
      } catch (error) {
        await sendError(error, reply);
      }
