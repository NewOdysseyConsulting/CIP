...
} from "@new-odyssey/lupercal-api";
import { Pool } from "pg";

const databaseUrl = process.env.LUPERCAL_DATABASE_URL;
const pollIntervalMs = Number.parseInt(
  process.env.LUPERCAL_WORKER_POLL_INTERVAL_MS ?? "1000",
  10,
);
const maxAttempts = Number.parseInt(process.env.LUPERCAL_WORKER_MAX_ATTEMPTS ?? "5", 10);
const retentionWindowHours = Number.parseInt(
  process.env.LUPERCAL_RETENTION_WINDOW_HOURS ?? "168",
  10,
);
const retentionSweepEveryLoops = Number.parseInt(
  process.env.LUPERCAL_RETENTION_SWEEP_EVERY_LOOPS ?? "300",
  10,
);

if (databaseUrl === undefined) {
  throw new Error("LUPERCAL_DATABASE_URL must be configured");
}
